...
class _af_loss:
  ...
  def _loss_unsupervised(self, inputs, outputs, aux):
    ...
    losses = {
      ...
      #"plddt_loop": get_plddt_loss(outputs, mask_1d=inputs['h3_mask']),
      #"pae_loop": get_pae_loss(outputs, mask_1d=inputs['h3_mask'], mask_1b=inputs['h3_mask']),
...
def get_plddt(outputs):
  ...
  #print(jax.make_jaxpr(lambda logits: jax.nn.softmax(logits))(logits))
  #jax.debug.print(probs, what='Probabilities: ')
  return jnp.sum(probs * bin_centers[None, :], axis=-1)

#def get_loop_plddt(outputs, h3_loop_index):
#    logits = outputs["predicted_lddt"]["logits"]
#    num_bins = logits.shape[-1]
#    bin_width = 1.0 / num_bins
#    bin_centers = jnp.arange(start=0.5 * bin_width, stop=1.0, step=bin_width)
