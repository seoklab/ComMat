...
if bare_metal_major != -1:
    modules = [CUDAExtension(
        ...
        sources=[
            "src/h3xsemble/utils/kernel/csrc/softmax_cuda.cpp",
            "src/h3xsemble/utils/kernel/csrc/softmax_cuda_kernel.cu",
        ],
        include_dirs=[
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)),
                'src/h3xsemble/utils/kernel/csrc/'
            )
        ],
        extra_compile_args={
    ...
    modules = [CppExtension(
        ...
        sources=[
            "src/h3xsemble/utils/kernel/csrc/softmax_cuda.cpp",
            "src/h3xsemble/utils/kernel/csrc/softmax_cuda_stub.cpp",
        ],
        extra_compile_args={
            'cxx': ['-O3'],
        }
